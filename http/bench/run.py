#!/usr/bin/env python3
"""Build and run the HTTP codec benchmark; print a markdown table. See README.md."""
...
OPS = ["parse_req", "parse_res", "encode_req", "encode_res"]
ENV = {**os.environ, "BEND_NO_TELEMETRY": "1", "NODE_NO_WARNINGS": "1"}
LLHTTP = subprocess.run(["brew", "--prefix", "llhttp"], capture_output=True, text=True, check=True).stdout.strip()

# name -> (build argv or None, run argv). Every program prints `op<TAB>ms<TAB>checksum` per op.
VARIANTS = {
    "C": (["clang", "-O2", f"-I{LLHTTP}/include", f"-L{LLHTTP}/lib", "-lllhttp", "-o", OUT / "c", "bench.c"], [OUT / "c"]),
    ...
    "Python": (None, ["uv", "run", "-q", "--with", "h11", "python", "bench.py"]),
