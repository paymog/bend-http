...
- Packages on the Elbow registry: `http@0.12.0` from `d7dfa35`; `wire@0.4.0` and `bytes@0.2.0` from `a00c480`; `zlib@0.1.0`, `url@0.4.0`, `json@0.3.0`, `dns@0.3.0`, `encoding@0.2.1`, and `router@0.1.1`. Fetch returns `Result` and headers are lists (`f73e022`). The README is `bf9e139`. The Bend hub reads the first comment in path order (`c9b30e0`).
...
- Laws: http 175, url 57, json 40, bytes 48, zlib 14, dns 18, encoding 13, router 3. Run `bend PROOF.bend` in the root and in each package folder.
...
- `fetch`, `exchange`, and streams read into `Bytes` and frame each read once: the head is parsed when it is whole, and chunk data runs are sliced whole. At 12 MiB a download takes about 90 ms in-process, down from 1.1 s with Content-Length and 27.5 s chunked, which re-framed the whole buffer after every read (`bench/fetch16.bend`). The cap is 16 MiB of body and 64 KiB of head; before, a body of exactly 16 MiB failed.
...
- [ ] **Make bodies `Bytes`.** `Req.body` and `Res.body` are still built as a `String`: on a 16 MiB upload that one copy and its free are most of the 0.23 s of CPU left, and a 12 MiB download still peaks at 434 MB of RSS. Changing them is a breaking release. `Req` and `Res` become linear, stream pieces become `Bytes`, and `zlib` needs a `Bytes` input.
