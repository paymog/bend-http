...
- Laws: http 155, url 57, json 40, bytes 48, zlib 14, dns 18, encoding 13, router 3. Run `bend PROOF.bend` in the root and in each package folder.
...
- [ ] **Move the client onto `Bytes`.** `fetch`, `exchange`, and streams still read into a reversed `String` and frame it again after each read. Read with `recv.words` and frame the way `serve` does, keeping `Res.body` a `String`.
- [ ] **Make bodies `Bytes`.** `Req.body` and `Res.body` are still built as a `String`: on a 16 MiB upload that one copy and its free are most of the 0.23 s of CPU left. Changing them is a breaking release. `Req` and `Res` become linear, stream pieces become `Bytes`, and `zlib` needs a `Bytes` input.
