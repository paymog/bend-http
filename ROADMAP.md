...
- Packages on the Elbow registry: `http@0.13.1` from `8846d0c`; `wire@0.4.0` and `bytes@0.2.0` from `a00c480`; `zlib@0.1.0`, `url@0.4.0`, `json@0.3.0`, `dns@0.3.0`, `encoding@0.2.1`, and `router@0.1.1`. Fetch returns `Result` and headers are lists (`f73e022`). The README is `bf9e139`. The Bend hub reads the first comment in path order (`c9b30e0`).
...
- `fetch`, `exchange`, and streams read into `Bytes` and frame each read once: the head is parsed when it is whole, and chunk data runs are sliced whole. At 12 MiB a download takes about 25 ms in-process and peaks at 32 MB of RSS. It took 1.1 s and 1.1 GB with Content-Length, and 27.5 s chunked, which re-framed the whole buffer after every read (`bench/fetch16.bend`). A `String` body alone held 434 MB. The cap is 16 MiB of body and 64 KiB of head; before, a body of exactly 16 MiB failed.
...
- [ ] **Share package types with callers.** Elbow publishes `http` with its own copies of `bytes`, `json`, and `url`, so `Json.Val` and `Url.Abs` from a separate `json` or `url` import do not match the ones `Http.json` and `Http.fetch` use. `Http.Body` re-exports the body type. Depend on the registry packages instead, or re-export the rest.
